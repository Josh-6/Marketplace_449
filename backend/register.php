...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: ../frontend/createacc.php');
    exit;
}
...
if ($username === '' || $password === '') {
    // missing fields
    header('Location: ../frontend/createacc.php?error=missing');
    exit;
}
if ($password !== $password_repeat) {
    header('Location: ../frontend/createacc.php?error=nomatch');
    exit;
}

// Password policy: at least 6 characters
if (strlen($password) < 6) {
    header('Location: ../frontend/createacc.php?error=weak');
    exit;
}
...
if (!$conn->query($create_users_sql)) {
    ...
    $conn->close();
    header('Location: ../frontend/createacc.php?error=server');
    exit;
}
...
if ($stmt->execute()) {
    $stmt->close();
    $conn->close();
    header('Location: ../frontend/index.php?registered=1');
    exit;
} else {
    ...
    $stmt->close();
    $conn->close();
    header('Location: ../frontend/createacc.php?error=exists');
    exit;
}
